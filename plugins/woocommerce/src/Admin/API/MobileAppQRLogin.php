...
class MobileAppQRLogin extends \WC_REST_Data_Controller {
	...
	public function exchange_token( $request ) {
		...
		// successfully created. Any subsequent call with the same token +
		// grant fails on the is_array() check above.
		delete_transient( $key );
		$this->delete_session_mapping_for_record( $token_data );
		$this->release_token_exchange_claim( $token_hash );
	...
	private function check_scan_rate_limit() {
		$ip    = $this->get_client_ip();
		$key   = self::RATE_LIMIT_PREFIX . 'scn_' . md5( $ip );
		$count = (int) get_transient( $key );

		if ( $count >= self::MAX_SCAN_PER_WINDOW ) {
			return false;
		}
	...
	private function check_approve_rate_limit( $user_id ) {
		$key   = self::RATE_LIMIT_PREFIX . 'apr_' . $user_id;
		$count = (int) get_transient( $key );

		if ( $count >= self::MAX_APPROVE_PER_WINDOW ) {
			return false;
		}
	...
	private function check_session_status_rate_limit( $session_id ) {
		$key   = self::RATE_LIMIT_PREFIX . 'ss_' . hash( 'sha256', $session_id );
		$count = (int) get_transient( $key );

		if ( $count >= self::MAX_SESSION_STATUS_PER_WINDOW ) {
			return false;
		}
