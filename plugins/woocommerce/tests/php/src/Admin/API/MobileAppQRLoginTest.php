...
class MobileAppQRLoginTest extends WC_REST_Unit_Test_Case {
	...
	/**
	 * Delete every transient created by the controller (token + consumed + rate limit).
	 */
	private function delete_all_qr_login_transients(): void {
		...
		// Remove Task 7 session-id → token-hash mapping transients written by /qr-login-scan.
		$wpdb->query(
